...
static int aw_dev_pid_59_5x9_pwr_on(struct aw_device *aw_dev,
		struct aw_data_container *data)
{
	...
	ret = aw87xxx_dev_i2c_write_byte(aw_dev,
		AW87XXX_PID_59_5X9_REG_DFT4R, 0x2C);
	if (ret < 0)
		return ret;

	return aw87xxx_dev_reg_update(aw_dev, data);
}
#endif
...
#if defined(CONFIG_TARGET_PRODUCT_LANCELOT)
	aw_dev->ops.pwr_on_func = aw_dev_pid_59_5x9_pwr_on;
#endif
