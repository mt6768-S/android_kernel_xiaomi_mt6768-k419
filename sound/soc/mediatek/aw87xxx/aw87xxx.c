...
static int aw87xxx_try_legacy_fw(struct aw87xxx *aw87xxx)
{
	int ret;

	mutex_lock(&aw87xxx->reg_lock);
	ret = aw87xxx_legacy_bin_load(aw87xxx->dev, &aw87xxx->acf_info);
	if (!ret)
		ret = aw87xxx_init_default_prof(aw87xxx);

	if (ret && aw87xxx->aw_dev.chipid == AW_DEV_CHIPID_59) {
#if defined(CONFIG_TARGET_PRODUCT_LANCELOT)
		aw87xxx_acf_profile_free(aw87xxx->dev, &aw87xxx->acf_info);
		ret = aw87xxx_legacy_aw87519_load(aw87xxx->dev,
			&aw87xxx->acf_info);
		if (!ret)
			ret = aw87xxx_init_default_prof(aw87xxx);
#endif
...
/* Legacy binary loader by vrdons start */
		ret = aw87xxx_try_legacy_fw(aw87xxx);
		if (!ret) {
			AW_DEV_LOGI(aw87xxx->dev,
				"legacy bin profiles loaded and initialized successfully");
			return;
		}

		AW_DEV_LOGE(aw87xxx->dev,
			"legacy firmware load failed, ret=%d", ret);
...
{
/* Legacy firmware loader by vrdons start */
	int ret;
	...
	if (prof_info->status == AW_ACF_WAIT) {
		ret = request_firmware_direct(&fw, aw87xxx->fw_name,
				aw87xxx->dev);
		if (ret < 0) {
			AW_DEV_LOGD(aw87xxx->dev,
				"optional ACF %s unavailable, trying legacy profiles",
				aw87xxx->fw_name);
		}
		aw87xxx_fw_load(fw, aw87xxx);
	}
/* Legacy firmware loader by vrdons end */
}
