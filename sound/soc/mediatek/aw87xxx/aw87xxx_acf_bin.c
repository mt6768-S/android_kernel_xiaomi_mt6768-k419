...
static const uint8_t aw87519_music_default[] = {
...
static const uint8_t aw87519_receiver_default[] = {
	0x69, 0x80, 0x69, 0xB7, 0x01, 0xF8, 0x02, 0x09,
	0x03, 0xC8, 0x04, 0x11, 0x05, 0x05, 0x06, 0x53,
	0x07, 0x4E, 0x08, 0x0B, 0x09, 0x08, 0x0A, 0x4B,
	0x60, 0x16, 0x61, 0x20, 0x62, 0x01, 0x63, 0x0B,
	0x64, 0xC5, 0x65, 0xA4, 0x66, 0x78, 0x67, 0xC4,
	0x68, 0x90,
};

static const uint8_t aw87519_hvload_default[] = {
	0x69, 0x80, 0x69, 0xB7, 0x01, 0xF0, 0x02, 0x09,
	0x03, 0xE8, 0x04, 0x11, 0x05, 0x10, 0x06, 0x43,
	0x07, 0x4E, 0x08, 0x03, 0x09, 0x08, 0x0A, 0x4A,
	0x60, 0x66, 0x61, 0xA0, 0x62, 0x01, 0x63, 0x0B,
	0x64, 0xD5, 0x65, 0xA4, 0x66, 0x78, 0x67, 0xC4,
	0x68, 0x90,
};

int aw87xxx_legacy_aw87519_load(struct device *dev,
		struct acf_bin_info *acf_info)
{
	static const char *const profile_names[] = {
		"Off", "Music", "Voice", "Fm", "Receiver",
	};
	static const uint8_t *const profile_data[] = {
		NULL, aw87519_music_default, aw87519_hvload_default,
		aw87519_hvload_default, aw87519_receiver_default,
	};
	static const uint32_t profile_lengths[] = {
		0, ARRAY_SIZE(aw87519_music_default),
		ARRAY_SIZE(aw87519_hvload_default),
		ARRAY_SIZE(aw87519_hvload_default),
		ARRAY_SIZE(aw87519_receiver_default),
	};
	struct aw_prof_info *prof_info = &acf_info->prof_info;
	int i;

	if (acf_info->aw_dev->chipid != 0x59)
		return -EINVAL;

	prof_info->prof_desc = devm_kzalloc(dev,
			sizeof(struct aw_prof_desc) * ARRAY_SIZE(profile_names),
			GFP_KERNEL);
	if (!prof_info->prof_desc)
		return -ENOMEM;

	prof_info->prof_name_list = devm_kzalloc(dev,
			sizeof(char [AW_PROFILE_STR_MAX]) * ARRAY_SIZE(profile_names),
			GFP_KERNEL);
	if (!prof_info->prof_name_list)
		return -ENOMEM;
	...
	for (i = 0; i < ARRAY_SIZE(profile_names); i++) {
		...
		snprintf(prof_info->prof_name_list[i], AW_PROFILE_STR_MAX,
			"%s", profile_names[i]);
		prof_info->prof_desc[i].prof_name = prof_info->prof_name_list[i];
		prof_info->prof_desc[i].prof_st = AW_PROFILE_OK;
		prof_info->prof_desc[i].data_container.len = profile_lengths[i];

		if (!profile_lengths[i])
			continue;

		copy_data = devm_kzalloc(dev, profile_lengths[i], GFP_KERNEL);
		if (!copy_data)
			return -ENOMEM;
		memcpy(copy_data, profile_data[i], profile_lengths[i]);
		prof_info->prof_desc[i].data_container.data = copy_data;
	}

	prof_info->count = ARRAY_SIZE(profile_names);
	prof_info->status = AW_ACF_UPDATE;
	AW_DEV_LOGI(dev, "loaded embedded AW87519 profiles");
...
int aw87xxx_legacy_bin_load(struct device *dev, struct acf_bin_info *acf_info)
{
	...
	if (acf_info->dev_index >= 0 && acf_info->dev_index <= 1)
		snprintf(index_sfx, sizeof(index_sfx), "_%d", acf_info->dev_index);
	...
	for (i = 0; i < ARRAY_SIZE(scenes); i++) {
		...
		if (ret < 0) {
			snprintf(fw_name, sizeof(fw_name), "aw87559_pid_%02x_%s%s.bin",
				acf_info->aw_dev->chipid, scenes[i], index_sfx);
			ret = request_firmware_direct(&fw, fw_name, dev);
		}
		if (ret < 0) {
