...
static ssize_t screen_info_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
    int res = 0;
...
static ssize_t screen_info_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", g_screen_info);
}
...
static ssize_t pscali_sec_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
    int data = 0;
    sscanf(buf, "%d", &data);
    printk("zch---pscali_sec_store  ct=%d\n", data);
    // sec cali
    ps_sec_cali_report(data);
    return count;
     }

 static ssize_t update_cali_data_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
    int data, res = 0;
    sscanf(buf, "%d", &data);
    printk("update_cali_data_store  ct=%d\n", data);
    ...
    printk("zch---update_cali_data_store send new CT =%d\n ", data);
    return count;
     }
...
DEVICE_ATTR_WO(pscali_sec);
DEVICE_ATTR_WO(update_cali_data);
DEVICE_ATTR_RW(screen_info);
