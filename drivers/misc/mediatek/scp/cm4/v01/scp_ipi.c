...
/* SCP ack beklemesi icin ust sinir. Saglikli donanimda ack
 * mikrosaniyeler icinde gelir; bu sinir fazlasiyla genis.
 */
	...
	unsigned int  len, unsigned int wait, enum scp_core_id scp_id)
{
	...
	if (wait) {
		...
		/* Bu dongu SINIRSIZDI: SCP takilirsa burada sonsuza kadar
		 * donuluyor, ustelik scp_ipi_mutex TUTULURKEN -- yani diger
		 * tum SCP kullanicilari da kilide takilip kaliyordu ve
		 * hicbir kayit birakmiyordu. Sinirla ve hatayi bildir;
		 * cagiranlar SCP_IPI_BUSY'yi zaten isliyor.
		 */
		while ((readl(SCP_GIPC_IN_REG) & (1<<scp_id)) > 0) {
			if (++spins > SCP_IPI_WAIT_MAX_SPINS) {
				pr_notice("[SCP] %s: id=%d ack gelmedi, vazgecildi\n",
					  __func__, id);
				if (scp_awake_unlock(scp_id) == -1)
