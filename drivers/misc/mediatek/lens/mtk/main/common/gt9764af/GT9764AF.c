...
static int i2c_write(u8 a_uAddr, u8 a_uData)
{
	int i4RetValue = 0;
	char puSendCmd[2] = { (char)(a_uAddr), (char)(a_uData) };
...
static u8 read_data(u8 addr)
{
	...
	LOG_INF("read_data(0x%02x) = 0x%02x\n", addr, get_byte);
	return get_byte;
}
...
static int s4AF_ReadReg(unsigned short *a_pu2Result)
{
	*a_pu2Result = (read_data(0x03) << 8) + (read_data(0x04) & 0xff);
	LOG_INF("s4AF_ReadReg result = 0x%04x\n", *a_pu2Result);
	return 0;
}
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	LOG_INF("s4AF_WriteReg(0x%04x) sent\n", a_u2Data);
	return 0;
}
...
static int AF_PowerDown(void)
{
	...
	LOG_INF("AF_PowerDown sequence start\n");

	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, PDSendCmdRelease, 2);
	if (i4RetValue < 0) {
		LOG_INF("PowerDown I2C send failed!!\n");
		return -1;
	}

	LOG_INF("AF_PowerDown sequence complete\n");
	return 0;
}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if ((a_u4Position > g_u4AF_MACRO) || (a_u4Position < g_u4AF_INF)) {
		LOG_INF("moveAF out of range: %lu (INF:%lu, MACRO:%lu)\n",
	...
	if (*g_pAF_Opened == 1) {
		...
		*g_pAF_Opened = 2;
		...
		LOG_INF("First move init: pos=%lu\n", g_u4CurrPosition);
	}
	...
	LOG_INF("moveAF [curr]=%lu [target]=%lu\n", g_u4CurrPosition, g_u4TargetPosition);
...
int GT9764AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Parking motor at mid-range\n");
		s4AF_WriteReg(512);
		msleep(20);
	}
...
void GT9764AF_SwitchToPowerDown(struct i2c_client *pstAF_I2Cclient)
{
	...
	LOG_INF("SwitchToPowerDown start\n");
	AF_PowerDown();
	LOG_INF("SwitchToPowerDown complete\n");
}
